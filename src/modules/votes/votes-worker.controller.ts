...
export class VotesWorkerController {
  ...
  handleVoteCast(@Payload() data: VoteEvent, @Ctx() context: RmqContext): void {
    this.addToBatch({ event: data, type: 'cast', context });
  }

  @EventPattern('vote.removed')
  handleVoteRemoved(
    @Payload() data: VoteEvent,
  ...
  private async flush() {
    ...
    try {
      ...
      for (const item of itemsToProcess) {
        const channel = item.context.getChannelRef() as unknown as Channel;
        const originalMsg =
          item.context.getMessage() as unknown as ConsumeMessage;

        channel.ack(originalMsg);
      }

      this.logger.log(
        `Successfully flushed batch of ${itemsToProcess.length} events.`,
      );
    } catch (error: unknown) {
      const errorMessage =
      ...
      this.logger.error(`Error flushing batch: ${errorMessage}`, errorStack);
